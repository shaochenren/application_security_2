...
def buy_card_view(request, prod_num=0):
    if request.method == 'GET':
        ...
        if director is not None:
            ...
            # fixed xss attack 1 by using escape
            context['director'] = html.escape(director)
        if prod_num != 0:
            try:
...
# We use csrf_protect to fix bug 2
# Get is supposed to handle sensitive data, hence in this case, it should not be used, we assume the ole partner already get the token
@csrf_protect
def gift_card_view(request, prod_num=0):
    context = {"prod_num" : prod_num}
    # As justified above, we should remove the gift function by GET
    if request.method == "GET":
    ...
    #     or request.method == "GET" and 'username' in request.GET:
        ...
        if prod_num == 0:
            prod_num = 1
        # Another method is to check the existence of csrfmiddlewaretoken
        # I fixed this for bug 2, but no longer needs it here since we use csrf_protect
        # Get vars from either post or get
        # if request.method == "POST" and request.POST.get('csrfmiddlewaretoken') is None:
        #         return redirect("/login.html")

        # user = request.POST.get('username', None) \
        #     if request.method == "POST" else request.GET.get('username', None)
        # amount = requesupdate framework without fixt.POST.get('amount', None) \
...
def use_card_view(request):
    ...
    if request.method == 'GET':
        ...
        # fix sql injection issue by using Django ORM
        # card_query = Card.objects.filter(data__contains=signature).values("id")
        all_card_list = Card.objects.all()
        card_query = []
        for x in all_card_list:
            if str(x.data).find(signature) != -1:
                card_query.append(x.id)
